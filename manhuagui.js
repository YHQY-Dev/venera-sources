...
class ManHuaGui extends ComicSource {
  ...
  parseComic(e) {
    ...
    let status = sl ? "连载" : "完结";
    let update = "";
    let updateon = e.querySelector(".updateon");
    if (updateon && updateon.text) {
      update = updateon.text.replace("更新于：", "").trim();
    }
    let tags = [status];
    if (update) tags.push(update);
  ...
  // explore page list — 用分类列表，比首页 multiPart 更稳
  explore = [
    {
      title: "最新更新",
      ...
      load: async (page) => {
        page = page || 1;
        let document = await this.getHtml(
          `${this.baseUrl}/list/index_p${page}.html`
        );
        let comics = document
          .querySelectorAll("#contList > li")
          .map((e) => this.parseComic(e) || this.parseSimpleComic(e))
          .filter((c) => c);
        return {
          comics,
  ...
  comic = {
    ...
    loadInfo: async (id) => {
      ...
      // 始终扁平化章节，避免探测/旧版把分组名当成 epId
      let chapters = new Map();
      for (let [_, groupChapters] of chaptersMap) {
        for (let [id, title] of groupChapters) {
          chapters.set(String(id), title);
        }
      }
      chapters = new Map(
        [...chapters].sort((a, b) => parseInt(a[0], 10) - parseInt(b[0], 10))
      );
    ...
    loadEp: async (comicId, epId) => {
      ...
      for (let s of scripts) {
        ...
        if (
          html.includes("function(p,a,c,k,e,d)") ||
          html.includes("LZString") ||
          html.includes("\\x65\\x76\\x61\\x6c")
        ) {
          script = html;
          break;
        }
      }
      if (!script && scripts[4]) script = scripts[4].innerHTML || "";
      if (!script) throw "chapter script not found";
