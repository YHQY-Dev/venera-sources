...
/**
 * 爱飒漫画（正版）→ 现跳转看漫 kanman.com
 * 列表 /{comicId}/ ；章节 getchapterlist；图片 getchapterinfov2.chapter_img_list
 */
class Isamanhua extends ComicSource {
  name = "爱飒漫画";
  key = "isamanhua";
  version = "1.0.0";
  minAppVersion = "1.0.0";
  ...
  static abs(href) {
    ...
    try {
      return new URL(href, Isamanhua.baseUrl + "/").href.replace(/^http:/i, "https:");
    } catch (_) {
      return href;
    }
  }
    ...
    for (let a of document.querySelectorAll("a[href]")) {
      ...
      comics.push(
        new Comic({
          id,
          title,
          cover: Isamanhua.abs(cover),
        })
      );
    }
    // fallback: cover images linking via parent
    if (comics.length < 3) {
      for (let img of document.querySelectorAll("img")) {
        ...
        seen[id] = true;
        comics.push(new Comic({ id, title, cover: Isamanhua.abs(src) }));
      }
    }
    return comics;
  }
  ...
  explore = [
    {
      ...
      load: async (page) => {
        ...
        let url =
          page <= 1
            ? `${Isamanhua.baseUrl}/`
            : `${Isamanhua.baseUrl}/sort/all.html?page=${page}`;
        let res = await Network.get(url, Isamanhua.headers);
        if (res.status !== 200) throw `Invalid status: ${res.status}`;
        let document = new HtmlDocument(res.body);
        let comics = Isamanhua.parseList(document);
  ...
  search = {
    load: async (keyword, options, page) => {
      page = page || 1;
      let url = `${Isamanhua.baseUrl}/search/?keywords=${encodeURIComponent(
        keyword
      )}&page=${page}`;
      let res = await Network.get(url, Isamanhua.headers);
      if (res.status !== 200) throw `Invalid status: ${res.status}`;
  ...
  comic = {
    loadInfo: async (id) => {
      ...
      let listRes = await Network.get(
        `${Isamanhua.baseUrl}/api/getchapterlist?comic_id=${id}`,
        {
          ...Isamanhua.headers,
          Accept: "application/json,*/*",
          Referer: `${Isamanhua.baseUrl}/${id}/`,
        }
      );
      ...
      // title/cover from page or first chapterinfov2
      let title = id;
      let cover = `https://image.yqmh.com/mh/${id}.jpg`;
      try {
        let pageRes = await Network.get(`${Isamanhua.baseUrl}/${id}/`, Isamanhua.headers);
      ...
      // order by order_num ascending if present
    ...
    loadEp: async (comicId, epId) => {
      ...
      epId = String(epId);
      let url = `${Isamanhua.baseUrl}/api/getchapterinfov2?comic_id=${comicId}&chapter_newid=${encodeURIComponent(
        epId
      )}`;
      let res = await Network.get(url, {
        ...Isamanhua.headers,
        Accept: "application/json,*/*",
        Referer: `${Isamanhua.baseUrl}/${comicId}/`,
      });
      if (res.status !== 200) throw `Invalid status: ${res.status}`;
      let data;
      try {
    ...
    onImageLoad: (url, comicId, epId) => {
      return {
        headers: {
          Referer: Isamanhua.baseUrl + "/",
          "User-Agent": Isamanhua.headers["User-Agent"],
        },
      };
    },
  };
