    ...
    explore = [
        {
            ...
            load: async (page) => {
                ...
                // 首页 HTML 微调时正则易空：用宽松链接兜底
                if (hotComics.length === 0 && latestComics.length === 0) {
                    let seen = {}
                    ...
                    for (let a of document.querySelectorAll("a[href*='/manhua/']")) {
                        ...
                        if (!m) continue
                        let id = m[1].startsWith("http") ? m[1] : ("https://www.ykmh.net" + m[1])
                        if (seen[id]) continue
                        ...
                        latestComics.push(new Comic({
                            id,
                            title,
                            cover: cover.startsWith("http") ? cover : ("https://www.ykmh.net" + cover),
                            tags: [],
                            description: ""
                        }))
                        if (latestComics.length >= 20) break
                    }
                }
